define(['rql/query', 'rql/js-array'], function (Query, jsArray) {
	...
	function checkQueryResultsEquality(query) {
		return fetch(new Request(`${testDatastoreUrl}?${query}`, {
				'method': 'GET',
				headers: datastoreRequestHeaders,
			})
		).then((datastoreResponse) => {
			const jsArrayResult = jsArray.query(query, {}, users);
			datastoreResponse.text().then((responseText) => {
				assert.strictEqual(responseText, JSON.stringify(jsArrayResult))
			});
	...
	registerSuite('RQL Implementations Compatability Test', {
		tests: {
			...
			'"limit" node is compatible'() {
				const limitQueryString = 'limit(5,10)';
			...
			'"and" node is compatible'() {
				const limitQueryString = 'and(select(id,firstName,lastName),sort(-lastName))';
				return checkQueryResultsEquality(limitQueryString);
			},
